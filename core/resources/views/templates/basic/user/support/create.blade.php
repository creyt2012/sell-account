                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form--label">@lang('Chủ đề')</label>
                                <input type="text" name="subject" value="{{old('subject')}}" class="form--control" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form--label">@lang('Mức độ')</label>
                                <select name="priority" class="form--control form-select" required>
                                    <option value="3">@lang('Cao')</option>
                                    <option value="2">@lang('Trung bình')</option>
                                    <option value="1">@lang('Thấp')</option>
                                </select>
                            </div>
                        </div> 
                        <div class="col-12 form-group">
                            <label class="form--label">@lang('Tin nhắn')</label>
                    ...
                    <div class="form-group">
                        <div class="text-end">
                            <button type="button" class="btn btn--base btn--sm addFile mt-4">
                                <i class="fa fa-plus"></i> @lang('Thêm mới')
                            </button>
                        </div> 
                        <div class="file-upload">
                            <label class="form--label">@lang('Tệp đính kèm')</label> 
                            <small class="text--danger">@lang('Tối đa 5 tệp có thể được tải lên'). @lang('Kích thước tải lên tối đa là') {{ ini_get('upload_max_filesize') }}</small>
                            <input type="file" name="attachments[]" id="inputAttachments" class="form--control mb-2"/>
                            <div id="fileUploadsContainer">
                                
                            </div>
                            <p class="ticket-attachments-message text-muted">
                                @lang('Phần mở rộng tệp được phép'): .@lang('jpg'), .@lang('jpeg'), .@lang('png'), .@lang('pdf'), .@lang('doc'), .@lang('docx')
                            </p>
                        </div>
                    </div>
