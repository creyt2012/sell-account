        ...
        <div class="col-lg-3">
            <div class="collapable-sidebar">
                <div class="collapable-sidebar__inner">
                    ...
                    <div class="card mb-4">
                        <div class="card-header border-bottom p-2 bg--base text--white text-center fw-bold">@lang('Thông tin của tôi')
                        </div>
                        <div class="card-body">
                    ...
                    <div class="card mb-4">
                        <div class="card-header border-bottom p-2 bg--base text-white text-center fw-bold">@lang('Hỗ trợ')
                        </div>
                        <div class="card-body">
                    ...
                    <div class="custom--card-two">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div>
    <p class="mb-0">@lang('Thanh toán')</p>
    <h4 class="my-1">{{ number_format(@$widget['total_payments'], 0, ',', '.') }} {{ $general->cur_sym }}</h4>
</div>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table--responsive--xl custom--table">
                            <thead>
                                <tr>
                                    <th>@lang('Cổng thanh toán | Trx')</th>
                                    <th>@lang('Ngày mua')</th>
                                    <th>@lang('Số tiền')</th>
                                    <th>@lang('Tổng thanh toán')</th>
                                ...
                                @forelse($latestDeposits as $deposit)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="td-wrapper">
                                                <span class="">
                                                    {{ __($general->cur_sym) }}{{ number_format(intval($deposit->amount), 0, ',', '.') }} + <span class="text--base"
                                                        title="@lang('charge')">{{ number_format(intval($deposit->charge), 0, ',', '.') }}
                                                    </span>
                                                </span>
                                                <strong class="d-block" title="@lang('Amount with charge')">
                                                    {{ number_format(intval($deposit->amount + $deposit->charge), 0, ',', '.') }}
                                                    {{ __($general->cur_text) }}
                                                </strong>
                                            </div>
                                        </td>
<td>
    <div class="td-wrapper">
        <span>
            1 {{ __($general->cur_text) }} = {{ number_format(intval($deposit->rate), 0, ',', '.') }}
            {{ __($deposit->method_currency) }}
        </span>
        <strong class="d-block">{{ number_format(intval($deposit->final_amo), 0, ',', '.') }}
